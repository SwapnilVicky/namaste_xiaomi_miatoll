...
FINAL_KERNEL_ZIP=VelvetPowerPulse-r1-KSU-MiAtoll-$(date '+%Y%m%d').zip
export PATH="$HOME/TxExcalibur/bin:$PATH"
export ARCH=arm64
export KBUILD_BUILD_HOST=Github-CI
export KBUILD_BUILD_USER=TxExcalibur
export KBUILD_COMPILER_STRING="$($HOME/TxExcalibur/bin/clang --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//')"

if ! [ -d "$HOME/TxExcalibur" ]; then
echo "Clang not found! Cloning..."
if ! git clone -q https://gitlab.com/GhostMaster69-dev/cosmic-clang --depth=1 --single-branch ~/TxExcalibur; then
